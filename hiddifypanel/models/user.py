...
class User(BaseAccount):
    ...
    def is_active(self) -> bool:
        ...
        if not self:
            ...
            is_active = False
        # watashi: quota boundary v12.2.55 - panel/hiddify.py:365 keeps a
        # user in the rendered configs only while usage_limit >
        # current_usage, so somebody sitting exactly on the limit was
        # already dropped from the cores while is_active still said True,
        # and remove_client() was therefore never called for them.
        elif (self.current_usage or 0) >= (self.usage_limit or 0):
            is_active = False
    ...
    def to_schema(self):
        ...
        return UserSchema().load(user_dict, unknown=EXCLUDE)
