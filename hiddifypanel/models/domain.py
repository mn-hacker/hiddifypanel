...
class Domain(db.Model):
    ...
    def internal_port_hysteria2(self):
        ...
        return int(hconfig(ConfigEnum.hysteria_port, self.child_id) or 443) + self.port_index
    ...
    def internal_port_mieru(self):
        if self.mode not in [DomainType.direct, DomainType.relay, DomainType.fake]:
            return 0
        # TODO: check validity of the range of the port
        return int(hconfig(ConfigEnum.mieru_port, self.child_id) or 443) + self.port_index
    ...
    def internal_port_naive(self):
        if self.mode not in [DomainType.direct, DomainType.relay, DomainType.fake]:
            return 0
        # TODO: check validity of the range of the port
        return int(hconfig(ConfigEnum.naive_port, self.child_id) or 443) + self.port_index
    ...
    def internal_port_special(self):
        if self.mode != DomainType.reality and "special" not in self.mode.value:
            return 0
        # TODO: check validity of the range of the port
        return int(hconfig(ConfigEnum.special_port, self.child_id) or 443) + self.port_index
